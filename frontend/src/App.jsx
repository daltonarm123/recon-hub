              ...
              <tbody>
                {(data?.reports || []).map((r) => (
                  <tr key={r.id}>
                    ...
                    {/* ✅ FIXED: view goes to FRONTEND route (readable), not /api JSON */}
                    <td style={td}>
                      <button
                        style={linkBtn}
      ...
      <div style={{ display: "grid", gap: 14 }}>
        ...
        >
          ...
          {meta.data?.report ? (
            ...
            >
              <div><b>Alliance:</b> {meta.data.report.alliance || "—"}</div>
              <div><b>Castles:</b> {meta.data.report.castles ?? "—"}</div>
              <div>
                <b>Defender DP:</b>{" "}
                {meta.data.report.defender_dp ? Number(meta.data.report.defender_dp).toLocaleString() : "—"}
              </div>
            </div>
          ) : null}
